...
```
```{r fvisdend corrected function}
##fvis_dend has an uncorrected bug that won't allow you to define colors of the bounding boxes correctly
##This is a corrected function taken from the following stack exchange thread
##https://stackoverflow.com/questions/76193584/how-to-adjust-rect-border-to-accept-multiple-colours-like-other-colour-palettes
...
fviz_dend2(combined_dtw_heir_clust_MV,               
          k=2, # Cut in k groups
          ylim = c(0, 200000),
          cex = 0.5, 
          show_labels = FALSE, 
          color_labels_by_k = FALSE, 
          k_colors = c("#d95f02", "#1b9e77"),
          rect = TRUE, 
          rect_border = c("#d95f02", "#1b9e77"), 
          rect_fill = TRUE, 
...
names(combined_dtw_heir_clust_MV) <- paste0("k_",2L:8L)
silhouette_vals = sapply(combined_dtw_heir_clust_MV, cvi, type = "valid") %>% 
  as.data.frame() %>% 
  pivot_longer(cols = everything(), names_to = "Cluster Number")
silhouette_vals = silhouette_vals[1:7,]
silhouette_vals$Clusters = c(2:8)
colnames(silhouette_vals)[2] = "Silhouette_Index"

silhouette_plot = ggplot(aes(x=Clusters, y=Silhouette_Index), data = silhouette_vals) 

silhouette_plot +
  geom_point(size = 4) +
  labs(x="Cluster Number" , y = "Silhouette Index") + 
    theme_bw() +
  theme(panel.border = element_rect(linewidth = 2, color = "black", fill = NA),
        ...
        axis.text = element_text(size = rel(1.2), face = "bold", color = "black"),
        legend.text = element_text(size = rel(1.2), face = "bold", color = "black"),
        legend.title = element_text(size = rel(1.2), face = "bold", color = "black"), 
...
print(
  ...
  labs(x=expression(bold("Distance from Otolith Core [")*bold(mu)*bold("m]")), y = paste(unique_elements[i], expression("["*mu*"g/g]"))) + 
    theme_bw() +
  theme(panel.border = element_rect(linewidth = 2, color = "black", fill = NA), 
        axis.ticks = element_line(linewidth = 2),
        axis.title.y = element_text(size = rel(2), face = "bold"),
...
#Plot of only Barium with 2 clusters

#Reversing the clusters for plotting
combined_DTW_melt_for_plot_MV$Rev_Clust_2 = as.factor(ifelse(combined_DTW_melt_for_plot_MV$Hier_Clust_2 == 1, 2, 1))

#Renaming Elements
combined_DTW_melt_for_plot_MV = combined_DTW_melt_for_plot_MV %>%
  mutate(Element_Ca = as.factor(Element_short))

combined_DTW_melt_for_plot_MV$Element_Ca = fct_recode(combined_DTW_melt_for_plot_MV$Element_Ca, "Li/Ca" = "Li", "Na/Ca" = "Na", "Mg/Ca" = "Mg", "Al/Ca" = "Al",  "P/Ca" = "P", "K/Ca" = "K", "Mn/Ca" = "Mn", "Cu/Ca" = "Cu", "Zn/Ca" = "Zn", "Rb/Ca" = "Rb", "Sr/Ca" = "Sr", "Ba/Ca" = "Ba", "Pb/Ca" = "Pb")


Ba_TimePeriod_plot =  ggplot(data=combined_DTW_melt_for_plot_MV[which(combined_DTW_melt_for_plot_MV$Element_short == "Ba"),], aes(x=microns, y=Ratio))

Ba_TimePeriod_plot +
  geom_line(aes(color = Location, group = OtoID), linewidth=0.2) +
  facet_grid(Rev_Clust_2~Time_Period) +
  scale_color_manual(values = c("#d01e89", "#9bd2a4", "#ff7f00", "#984ea3", "#e41a1c", "#377eb8", "#4daf4a", "#cab2d6"), name = "Sample Location") +
  labs(x=expression(bold("Distance from Otolith Core [")*bold(mu)*bold("m]")), y = expression(bold("Ba/Ca [")*bold(mu)*bold("g/g]"))) + 
    theme_bw() +
  theme(panel.border = element_rect(linewidth = 2, color = "black", fill = NA),
        strip.background=element_rect(linewidth=2),
        axis.ticks = element_line(linewidth = 2),
...
palette_num = brewer.pal(name = "Dark2", n = grp_num)
...
#Reversing the clusters for plotting
means_data$Rev_Clust_2 = as.factor(ifelse(means_data$Hier_Clust_2 == 1, 2, 1))

means_data$Rev_Clust_2 = as.factor(ifelse(means_data$Hier_Clust_2 == 1, 2, 1))

####making the percent plot alone for 2 groups
agegroup_percent_plot_MV = ggplot(means_data, aes(x = Time_Period))

agegroup_percent_plot_MV +
  geom_bar(aes(fill = Rev_Clust_2), position = "fill") +
  scale_fill_manual(values = c("#1b9e77", "#d95f02"), name="Cluster") + 
  scale_y_continuous(labels = scales::percent) +
  labs(x="\nTime Period Group", y = "Cluster %") +
  theme_bw() +
  theme(panel.border = element_rect(linewidth = 2, color = "black", fill = NA), 
        strip.background=element_rect(linewidth=2),
        axis.ticks = element_line(linewidth = 2),
...
###Plot of means for entire transect by cluster

#melt to long form for plotting
means_data_long = means_data %>% 
  pivot_longer(cols = Li_ug_g_m7:Pb_ug_g_m208, values_to = "Ratio", names_to = "Element") %>% 
  select(-c(Occupation_layer:Hier_Clust_8))

means_data_long$Element_short = sub("\\_.*", "", means_data_long$Element)

means_data_long$Element_Ca = fct_recode(means_data_long$Element_short, "Li/Ca" = "Li", "Na/Ca" = "Na", "Mg/Ca" = "Mg", "Al/Ca" = "Al",  "P/Ca" = "P", "K/Ca" = "K", "Mn/Ca" = "Mn", "Cu/Ca" = "Cu", "Zn/Ca" = "Zn", "Rb/Ca" = "Rb", "Sr/Ca" = "Sr", "Ba/Ca" = "Ba", "Pb/Ca" = "Pb")

means_data_metals = subset(means_data_long, Element_short %in% c("Li", "Al", "Cu", "Zn", "Pb"))

metals_2clust_boxplot = ggboxplot(means_data_metals, x = "Rev_Clust_2", y = "Ratio",
              color = "Rev_Clust_2", palette = c("#1b9e77", "#d95f02"), short.panel.labs = TRUE)
# Use only p.format as label. Remove method name.
facet(metals_2clust_boxplot + stat_compare_means(
 aes(label = paste0("p = ", after_stat(p.format)))
), facet.by = "Element_Ca", nrow = 1, scales = "free") +
  labs(x= "Cluster", y = expression(bold("Element/Ca ["*mu*"g/g]")), color = "Cluster") +
  theme(panel.border = element_rect(linewidth = 2, color = "black", fill = NA), 
        strip.background=element_rect(linewidth=2),
        axis.ticks = element_line(linewidth = 2),
        axis.title.y = element_text(size = rel(2), face = "bold"),
...
descr_stats_juvy = combined_DTW_melt_for_plot_MV #subset(combined_DTW_melt_for_plot_MV, microns <=1000)

descr_stats_juvy = descr_stats_juvy %>% 
  group_by(OtoID, Element_Ca, Rev_Clust_2) %>% 
  summarize(Ratio = mean(Ratio))

descr_stats_juvy_metals = subset(descr_stats_juvy, Element_Ca %in% c("Li/Ca", "Al/Ca", "Cu/Ca", "Zn/Ca", "Pb/Ca"))

juvy_metals_2clust_boxplot = ggboxplot(descr_stats_juvy_metals, x = "Rev_Clust_2", y = "Ratio",
              color = "Rev_Clust_2", palette = c("#1b9e77", "#d95f02"), short.panel.labs = TRUE)
# Use only p.format as label. Remove method name.
facet(juvy_metals_2clust_boxplot + stat_compare_means(aes(label = paste0("p = ", after_stat(p.format))), label.x.npc = 0.2, label.y.npc = .9), facet.by = "Element_Ca", nrow = 1, scales = "free") +
  labs(x= "Cluster", y = expression(bold("Element/Ca ["*mu*"g/g]")), color = "Cluster") +
  theme(panel.border = element_rect(linewidth = 2, color = "black", fill = NA), 
        strip.background=element_rect(linewidth=2),
        axis.ticks = element_line(linewidth = 2),
...
```{r pulling centroids for plotting}

OtoID = names(combined_dtw_heir_clust_MV@centroids)

Hier_Clust_2 = c(2,1)

centroids = as.data.frame(cbind(OtoID, Hier_Clust_2))

centroid_data = subset(combined_DTW_melt_for_plot_MV, OtoID == centroids$OtoID)

# #Reversing the clusters for plotting
# combined_DTW_melt_for_plot_MV$Rev_Clust_2 = as.factor(ifelse(combined_DTW_melt_for_plot_MV$Hier_Clust_2 == 1, 2, 1))
# 
# centroid_data$Rev_Clust_2 = as.factor(ifelse(centroid_data$Hier_Clust_2 == 1, 2, 1))
...
# centroid_data$Element_Ca = fct_recode(centroid_data$Element_Ca, "Li/Ca" = "Li", "Na/Ca" = "Na", "Mg/Ca" = "Mg", "Al/Ca" = "Al",  "P/Ca" = "P", "K/Ca" = "K", "Mn/Ca" = "Mn", "Cu/Ca" = "Cu", "Zn/Ca" = "Zn", "Rb/Ca" = "Rb", "Sr/Ca" = "Sr", "Ba/Ca" = "Ba", "Pb/Ca" = "Pb")

centroid_data_subset = subset(centroid_data, Element_short %in% c("Al", "Ba", "Mg", "Mn", "Na", "Rb", "Sr"))
centroid_plot_data_subset = subset(combined_DTW_melt_for_plot_MV, Element_short %in% c("Al", "Ba", "Mg", "Mn", "Na", "Rb", "Sr"))

#Centroid Plot
centroid_plot = ggplot(data = centroid_plot_data_subset, aes(x=microns, y=Ratio))

centroid_plot +
  geom_line(aes(color = Rev_Clust_2, group = OtoID), linewidth=0.2, alpha = 0.3) +
  facet_grid(Element_Ca~Rev_Clust_2, scales = "free_y") +
  guides(x = guide_axis(n.dodge = 2)) +
  scale_color_manual(values = c("#1b9e77", "#d95f02"), name="Cluster") +
  geom_line(data = centroid_data_subset, aes(x=microns, y=Ratio), color="black", linewidth = 1) +
  theme_bw()+
  guides(color = guide_legend(override.aes = list(size=5))) +
  labs(x="Microns from Otolith Core", y = expression(bold("Element/Ca ["*mu*"g/g]"))) +
  theme(panel.border = element_rect(linewidth = 2, color = "black", fill = NA), 
        strip.background=element_rect(linewidth=2),
        axis.ticks = element_line(linewidth = 2),
        axis.title.y = element_text(size = rel(2), face = "bold"),
...
layer_percent_plot_MV +
  ...
  scale_fill_brewer(values = Dark2, name="Occupation\nLayer") +
  scale_y_continuous(labels = scales::percent) +
